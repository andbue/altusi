...
class Figure():
    ...
    def __init__(self, z, titles, fig_orientation, figure_font_size, figure_title, stat_mfwords,
        ...
        self.delta_algorithm = const._fields[delta_choice]
    ...
    def show(self):
        ...
        plt.tight_layout(2)
        #if self.figure_show:
            #plt.savefig(self._result_filename(self.figure_title, self.save_sep, self.stat_mfwords) + ".svg")
            #plt.show()
        #svg = self.gensvg(plt)
        return dendro_data, plt#, svg

    def save(self, plt):
        plt.savefig(self._result_filename(self.figure_title, self.save_sep, self.stat_mfwords) + ".svg")

    def genimage(self, plt, format="svg"):
        output = io.BytesIO()
...
def main():
    ...
    if cfg.cfg["stat.evaluate"]:
        ...
        print("\nAlgorithm: ", const._fields.replace("_", " ").title())
